...
description: 'TicketKiosk and Curation contracts deployed individually for each event'
icon: 'ticket'
---

# Per-Event Contracts

The RTA protocol deploys dedicated contract instances for each event to handle event-specific functionality. This modular approach isolates event logic, reduces gas costs for global operations, and enables customized configurations per event.
...
- High gas costs for full event setup (~4.3M gas)
- CREATE2 deployment requires careful salt management
- Failed deployments waste gas without recovery

### Contract Complexity

- Each event multiplies the attack surface
- Inconsistent implementations across events
- Difficult to upgrade individual event contracts

### Economic Risks

- Immediate ticket revenue distribution reduces protocol TVL
- Curation fee extraction reduces creator earnings
- No mechanism for dispute resolution in service provider conflicts

The per-event contract architecture provides the modularity and customization necessary for diverse event types while maintaining integration with the broader protocol ecosystem. This design choice prioritizes event creator autonomy and service provider flexibility over protocol-wide optimization.
